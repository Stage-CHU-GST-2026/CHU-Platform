<script lang="ts">
    ...
    let { children, prompt, scrollContainerRef = $bindable(null) }: Props = $props();
</script>

<div class="relative flex flex-1 flex-col h-full overflow-hidden min-h-0">
    <!-- Messages slot: scrollable container with bottom padding so content scrolls above floating prompt -->
    <div
    ...
    <!-- Floating prompt snippet slot: ALWAYS VISIBLE AT BOTTOM -->
    {#if prompt}
