...
<div class="msg-row assistant my-2">
    <div class="w-full">
        ...
        <div class="msg-bubble assistant !p-0 !max-w-[85%] overflow-hidden flex flex-col transition-shadow">
            <div class="flex items-start gap-4 p-5">
                <div class="w-10 h-10 rounded-lg bg-surface-elevated border border-border flex items-center justify-center shrink-0 text-accent">
                    <IconClipboardList size={22} stroke={1.5} />
                </div>
                
                <div class="flex-1 flex flex-col pt-0.5">
                    <h3 class="text-text-primary font-semibold text-[15px] mb-1 tracking-tight">{planArtifact.title || planArtifact.filename}</h3>
                    <p class="text-text-secondary text-[14px] leading-relaxed mb-4 max-w-[95%]">
                        {planArtifact.description || 'A detailed plan has been created. Review it and let me know if we should proceed.'}
                    </p>
            ...
            <div class="bg-surface px-5 py-2.5 border-t border-border-subtle flex items-center justify-between">
